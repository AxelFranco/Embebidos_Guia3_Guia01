...
int main()
{
    ...
    while (true) {
        ...
        for(conversionStep = 1; conversionStep <= NUMBER_OF_BITS; conversionStep++)
        {
            DACOutput = digitalToAnalogConverter();
            comparatorOutput = analogComparator();
            DACInput[NUMBER_OF_BITS - conversionStep] = iterativeConversionControllerStep();
...
void resetIterativeConversionController()
{
    ...
    uartUsb.write("DAC Input reseted\r\n\r\n", 20);
}
...
void showConversionStatus()
{
    ...
    sprintf(str, "%.2f", analogInput);
    uartUsb.write(str, strlen(str));
    sprintf(str, "DAC Output: %.2f\r\n", DACOutput);
    uartUsb.write(str, strlen(str));
